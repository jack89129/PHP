...
class Overige_KasboekController extends Jaycms_Controller_Action
{     
    ...
    {    
        parent::init();
        $this->view->page_title = _t("Kasboek");
    ...
    {   
        ...
        $start_invoice['result'] = $invoiceModel->findInvoicesForOverige('paid', $employeeId, $date_from, $date_to, $start_invoice['total'], $start_invoice['sum'], $start_invoice['sum_no_vat'], InvoicePaymentModel::PAYMENT_METHOD_CASH);
        $start_purchase['result'] = $purchaseModel->findPurchasesForOverige('paid', $date_from, $date_to, $start_purchase['total'], $start_purchase['sum'], $start_invoice['sum_no_vat'], PurchasePaymentModel::PAYMENT_METHOD_CASH);
        
        $date_from = $today;
        $date_to = $today;
        $end_invoice = array( 'result' => array(), 'sum' => 0, 'sum_no_vat' => 0);     
        $end_purchase = array( 'result' => array(), 'sum' => 0, 'sum_no_vat' => 0);     
        $end_invoice['result'] = $invoiceModel->findInvoicesForOverige('paid', $employeeId, $date_from, $date_to, $end_invoice['total'], $end_invoice['sum'], $end_invoice['sum_no_vat'], InvoicePaymentModel::PAYMENT_METHOD_CASH);
        $end_purchase['result'] = $purchaseModel->findPurchasesForOverige('paid', $date_from, $date_to, $end_purchase['total'], $end_purchase['sum'], $end_purchase['sum_no_vat'], PurchasePaymentModel::PAYMENT_METHOD_CASH);
    ...
    public function searchUnpaidAction(){
        ...
        if ( $amount > 0 ) {
            $invoiceModel = new InvoiceModel(); 
            $unpaid = $invoiceModel->findUnpaidAllInvoices($amount);
            /*if ( count($unpaid) == 1 ) {
                $invoiceModel->setPaidInvoice($unpaid[0]->number);
            }*/
        } else {
            $purchaseModel = new PurchaseModel(); 
            $unpaid = $purchaseModel->findUnpaidAllPurchases(-1 * $amount);
            /*if ( count($unpaid) == 1 ) {
