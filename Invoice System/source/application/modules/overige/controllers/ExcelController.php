...
class Overige_ExcelController extends Jaycms_Controller_Action
{     
    ...
    {   
        ...
        $invoices['result'] = $invoiceModel->findInvoicesForOverige('paid', $employeeId, $date_from, $date_to, $invoices['total'], $invoices['sum'], $invoices['sum_no_vat']);
        $purchases['result'] = $purchaseModel->findPurchasesForOverige('paid', $date_from, $date_to, $purchases['total'], $purchases['sum'], $purchases['sum_no_vat']);
    ...
    {   
        ...
        $result = $bankModel->getMonthBankboek($year_month, $total, $sum, $sum_no_vat);
        
        setlocale(LC_ALL, 'nl_NL');
        $this->view->result = $result;
        $this->view->kas_date = strftime('%B %Y', $kas_date);
        $this->view->startsaldo = $bankModel->getLatestBalance($from);
        $this->view->endsaldo = $bankModel->getLatestBalance($to);    
        $this->view->afsch = $bankModel->getAfschrift($date);     
        $this->view->period = $period;  
        
        $this->generateMonthPDF('Bankboek-maandafschriften-' . date('m-Y', $kas_date) . '.pdf', 'D');
        die();                                  
    }
    ...
    protected function generateMonthPDF($name, $destination){
        ...
        $content = $this->view->render('excel/month.phtml');
