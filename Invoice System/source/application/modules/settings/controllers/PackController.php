...
class Settings_PackController extends Jaycms_Controller_Action {
    ...
    public function indexAction(){
        ...
        if( ($tabs = $cache->load($cacheId)) === false ){
            $packModel = new SettingPackModel();
            $tabs = array();
    ...
    public function viewAction(){
        ...
        $pack = new SettingPack($id);

        if( $pack->exists() ){
            // do nothing
            Utils::activity('view', 'pack', $pack->id);
        }else{
            ...
            Utils::activity('new', 'pack', $pack->id);
            $this->getRequest()->setParam('edit', 1);
            //$this->_redirect("/settings/pack/edit/id/" . $pack->id);
        }

        $readonly = $pack->status == SettingPackModel::STATUS_FINAL ? true : $readonly;

        if( !$pack->products ){
            $product = new SettingPackProduct();
    ...
    public function saveAction(){
        ...
        $is_update_name = $this->_getParam('update_name');
        if ( $is_update_name == "1" ) {
            $packModel = new SettingPackModel();
            $result = $packModel->findPackByEmp($packParam['employee_id']);
            if ( !empty($result) && $packParam['employee_id'] != $pack->employee_id ) {
                throw new Exception(_t("Elke werknemer kan maar 1 algemene laadlijst hebben."));
            } 
        }
     
        $pack->number = $packParam['number'];
        if( $pack->status != SettingPackModel::STATUS_FINAL ){
        ...
        foreach( $newProducts as $product ){
            $newProduct = new SettingPackProduct();
    ...
    public function unfinalAction(){
        $id = (int) $this->_getParam('id', 0);
        $this->unfinalize($id);
        $this->_helper->json(array('redirect' => '/settings/pack/view/id/' . ((int) $id)));
    }

    private function unfinalize($id){
        $pack = new SettingPack($id);
        ...
        try {
            $pack->status = SettingPackModel::STATUS_NEW;
            $pack->save('status');

            $log = new Log();
            $log->source_type = LogModel::SOURCE_TYPE_PACK;
            $log->source_id = $pack->id;
            $log->event = LogModel::EVENT_PACK_UNFINAL;
            $log->save();
    ...
    private function finalize($id){
        ...
        $packModel = new SettingPackModel();
        $result = $packModel->findFinalPack($pack->employee_id);
        if ( !empty($result) ) {
            throw new Exception(_t("Elke werknemer kan maar 1 algemene laadlijst hebben."));
        } 
        ...
        try {
            ...
            $pack->status = SettingPackModel::STATUS_FINAL;
            $pack->save('status');
    ...
    public function deleteAction(){
        ...
        $this->delete($id);
        $this->_helper->json(array('redirect' => $this->view->baseUrl() . '/settings/pack'));
    }
